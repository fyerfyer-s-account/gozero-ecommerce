...
func (s *RmqServer) Start() {
    s.logger.Info(s.ctx, "Starting Marketing RMQ server...", nil)
    ...
    s.logger.Info(s.ctx, "Marketing RMQ server started successfully", nil)
}

func (s *RmqServer) Stop() {
    s.logger.Info(s.ctx, "Stopping Marketing RMQ server...", nil)
    ...
    s.logger.Info(s.ctx, "Marketing RMQ server stopped successfully", nil)
}