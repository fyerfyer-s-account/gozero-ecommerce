...
type (
	...
	Users struct {
		...
		IsAdmin     int64          `db:"is_admin"` // is admin or not
	}
)
...
func (m *defaultUsersModel) Insert(ctx context.Context, data *Users) (sql.Result, error) {
	...
	ret, err := m.conn.ExecCtx(ctx, query, data.Username, data.Password, data.Phone, data.Email, data.Nickname, data.Avatar, data.Gender, data.MemberLevel, data.Status, data.IsAdmin)
	return ret, err
}

func (m *defaultUsersModel) Update(ctx context.Context, newData *Users) error {
	query := fmt.Sprintf("update %s set %s where `id` = ?", m.table, usersRowsWithPlaceHolder)
	_, err := m.conn.ExecCtx(ctx, query, newData.Username, newData.Password, newData.Phone, newData.Email, newData.Nickname, newData.Avatar, newData.Gender, newData.MemberLevel, newData.Status, newData.IsAdmin, newData.Id)
	return err
}
