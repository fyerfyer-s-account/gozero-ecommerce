...
func (m *customPointsRecordsModel) FindByUserId(ctx context.Context, userId int64, page, pageSize int32) ([]*PointsRecords, error) {
	...
	query := fmt.Sprintf("select %s from %s where `user_id` = ? order by created_at desc limit ?, ?", pointsRecordsRows, m.table)
	err := m.QueryRowsNoCacheCtx(ctx, &records, query, userId, offset, pageSize)
	return records, err
}
