...
func TestSelectAllLogic_SelectAll(t *testing.T) {
    ...
    tests := []struct {
        name    string
        req     *cart.SelectAllRequest
        wantErr error
        check   func(*testing.T)
    }{
        {
            name: "Select all successfully",
            req: &cart.SelectAllRequest{
                UserId: 1,
            },
            wantErr: nil,
            check: func(t *testing.T) {
                ...
                assert.NoError(t, err)
                assert.Equal(t, int64(3), stats.SelectedQuantity)
                assert.Equal(t, float64(400), stats.SelectedAmount)
            },
        },
        {
    ...
    for _, tt := range tests {
        t.Run(tt.name, func(t *testing.T) {
            l := NewSelectAllLogic(context.Background(), ctx)
            resp, err := l.SelectAll(tt.req)
