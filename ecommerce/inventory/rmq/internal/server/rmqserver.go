...
import (
    ...
    "github.com/fyerfyer/gozero-ecommerce/ecommerce/inventory/rmq/internal/svc"
...
func (s *RmqServer) Start() {
    s.logger.Info(s.ctx, "Starting Inventory RMQ server...", nil)
    ...
    s.logger.Info(s.ctx, "Inventory RMQ server started successfully", nil)
}

// Stop implements service.Service
func (s *RmqServer) Stop() {
    s.logger.Info(s.ctx, "Stopping Inventory RMQ server...", nil)
    ...
    s.logger.Info(s.ctx, "Inventory RMQ server stopped successfully", nil)
}
