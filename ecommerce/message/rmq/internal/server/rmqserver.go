...
import (
    ...
    "github.com/fyerfyer/gozero-ecommerce/ecommerce/message/rmq/internal/svc"
...
func (s *RmqServer) Start() {
    s.logger.Info(s.ctx, "Starting Message RMQ server...", nil)
    ...
    s.logger.Info(s.ctx, "Message RMQ server started successfully", nil)
}

func (s *RmqServer) Stop() {
    s.logger.Info(s.ctx, "Stopping Message RMQ server...", nil)
    ...
    s.logger.Info(s.ctx, "Message RMQ server stopped successfully", nil)
}